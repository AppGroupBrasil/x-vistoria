...
import {
  ...
  CheckCircle2, Shield, Loader2, AlertTriangle
} from 'lucide-react'
...
export default function PortalEspacoPage() {
  ...
  useEffect(() => {
    if (!token) return
    axios
      .get(`${API_URL}/api/v1/publico/portal/espaco/${token}`)
      .then((res) => {
        setEspaco(res.data.espaco)
        setVisitas(res.data.visitas || [])
        setPendencias(res.data.pendencias || [])
      })
      .catch((err: any) => setError(extrairErro(err?.response?.data || err, 'Espaço não encontrado ou link inválido.')))
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-2">Espaço não encontrado</h1>
          <p className="text-gray-500 text-sm">{error || 'O QR Code pode estar desatualizado ou o espaço foi desativado.'}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-gradient-to-br from-emerald-600 to-teal-700 text-white">
        <div className="max-w-2xl mx-auto px-4 py-8">
          <div className="flex items-center gap-3 mb-4">
            {espaco.empresa_logo ? (
              <img src={espaco.empresa_logo} alt="" className="w-12 h-12 rounded-xl bg-white/20 object-contain p-1" />
            ) : (
            ...
            <div>
              ...
              <h1 className="text-xl font-bold">{espaco.condominio_nome}</h1>
            </div>
          </div>

          {/* Nome do espaço */}
          <div className="bg-white/15 rounded-xl px-4 py-3 backdrop-blur-sm mb-4">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-emerald-100 text-sm">
            {espaco.endereco && (
              <span className="flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5" /> {espaco.endereco}
                {espaco.cidade && `, ${espaco.cidade}/${espaco.estado}`}
              </span>
            )}
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        <section>
          <h2 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
            <ClipboardCheck className="w-5 h-5 text-emerald-500" /> Vistorias — {espaco.nome}
          </h2>

          {visitas.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-400">
              <ClipboardCheck className="w-8 h-8 mx-auto mb-2 opacity-40" />
              <p className="text-sm">Nenhuma vistoria concluída neste espaço.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
                        ...
                        {v.supervisor_nome && <span>Funcionário: {v.supervisor_nome}</span>}
        ...
        <div className="text-center py-6 text-xs text-gray-400 border-t border-gray-100">
          <p>Portal de transparência — {espaco.empresa_nome}</p>
